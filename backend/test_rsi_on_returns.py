#!/usr/bin/env python3
"""Test RSI calculated on percentage returns, then smoothed with VAR."""
...
def calculate_rsi_on_returns(ticker):
    """Calculate RSI on percentage returns (not price changes)."""
    ...
    returns = close_price.pct_change() * 100  # Convert to percentage

    # Calculate RSI on returns (Wilder's method)
    rsi_length = 14
...
print("="*80)
print("RSI ON PERCENTAGE RETURNS + VAR SMOOTHING TEST")
print("="*80)
print("Method: Close -> Returns (%) -> RSI (14) -> VAR (5)")
...
for ticker in tickers:
    ...
    try:
        last_date, last_close, rsi_ma = calculate_rsi_on_returns(ticker)
        ...
        if rsi_ma is not None:
            ...
            print(f"  RSI-MA (Returns + VAR): {rsi_ma:.2f}")
