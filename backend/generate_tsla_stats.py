#!/usr/bin/env python3
"""
Generate complete 4H and Daily statistics for TSLA (Tesla)
following the exact pattern used for GLD, SLV, NVDA, MSFT, GOOGL, AAPL
"""
...
def generate_guidance(personality_data: Dict, ticker: str) -> Dict:
    ...
    if ticker == 'TSLA':
        name = "Tesla, Inc."
        entry = "Enter LONG when momentum is confirmed (percentile > 75). TSLA shows strong trending behavior. Avoid catching falling knives."
        avoid = "Avoid trading in 25-50 range - weak signals. Never fight the trend. Be cautious during earnings."
        special = "TSLA is extremely volatile and momentum-driven. Best for trend following strategies. Use wider stops. Works well for 7-14 day momentum plays. High news sensitivity."
    else:
        name = ticker
        entry = "Trade based on momentum and trend direction"
        avoid = "Middle ranges and choppy periods"
        special = "N/A"
...
def main():
    ...
    output_file = '/workspaces/New-test-strategy/backend/tsla_statistics.json'
