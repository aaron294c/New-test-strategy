...
class PercentileBinStats:
    ...
    std_return_1d: float
    std_return_5d: float
    std_return_10d: float

    # Risk metrics
    downside_risk_1d: float  # Std of negative returns
    upside_potential_1d: float  # Mean of positive returns

    # Percentiles of return distribution
    pct_5_return_1d: float  # 5th percentile (worst case)
    pct_95_return_1d: float  # 95th percentile (best case)
...
class PercentileForwardMapper:
    ...
    def calculate_empirical_bin_stats(self, df: pd.DataFrame) -> Dict[int, PercentileBinStats]:
        ...
        for bin_idx, (pmin, pmax, label) in enumerate(self.percentile_bins):
            ...
            ret_1d = bin_data['ret_1d'].dropna()
            ret_5d = bin_data['ret_5d'].dropna() if 'ret_5d' in bin_data else pd.Series([0])
            ret_10d = bin_data['ret_10d'].dropna() if 'ret_10d' in bin_data else pd.Series([0])

            # Downside risk (std of negative returns)
            downside = ret_1d[ret_1d < 0]
            downside_risk = downside.std() if len(downside) > 0 else 0

            # Upside potential (mean of positive returns)
            upside = ret_1d[ret_1d > 0]
            ...
            stats_obj = PercentileBinStats(
                ...
                bin_max=pmax,
                count=len(bin_data),
                mean_return_1d=ret_1d.mean(),
                mean_return_5d=ret_5d.mean(),
                mean_return_10d=ret_10d.mean(),
                median_return_1d=ret_1d.median(),
                median_return_5d=ret_5d.median(),
                median_return_10d=ret_10d.median(),
                std_return_1d=ret_1d.std(),
                std_return_5d=ret_5d.std(),
                std_return_10d=ret_10d.std(),
                downside_risk_1d=downside_risk,
                upside_potential_1d=upside_potential,
                pct_5_return_1d=ret_1d.quantile(0.05),
                pct_95_return_1d=ret_1d.quantile(0.95),
            )
    ...
    def percentile_first_forecast(self, current_bin: int, current_percentile: float, horizon: int) -> PercentileFirstForecast:
        ...
        if predicted_bin in self.bin_lookup:
            ...
            if horizon <= 3:
                expected_return = future_bin_stats.mean_return_1d
                downside = future_bin_stats.pct_5_return_1d
                upside = future_bin_stats.pct_95_return_1d
            elif horizon <= 7:
                expected_return = future_bin_stats.mean_return_5d
                downside = future_bin_stats.pct_5_return_1d * (horizon / 1)
                upside = future_bin_stats.pct_95_return_1d * (horizon / 1)
            else:
                expected_return = future_bin_stats.mean_return_10d
                downside = future_bin_stats.pct_5_return_1d * (horizon / 1)
                upside = future_bin_stats.pct_95_return_1d * (horizon / 1)
        else:
            expected_return = 0
    ...
    def markov_forecast(self, current_bin: int, horizon: int) -> float:
        ...
        for j, prob in enumerate(transition_probs):
            if j in self.bin_lookup:
                ...
                if horizon == 1:
                    expected_return += prob * bin_stats.mean_return_1d
                elif horizon == 5:
                    expected_return += prob * bin_stats.mean_return_5d
                elif horizon == 10:
                    expected_return += prob * bin_stats.mean_return_10d
    ...
    def kernel_forecast(self, df: pd.DataFrame, current_percentile: float, horizon: int, bandwidth: float = 10.0) -> KernelForecast:
        ...
        return KernelForecast(
            ...
            forecast_1d=forecasts.get(1, 0),
            forecast_5d=forecasts.get(5, 0),
            forecast_10d=forecasts.get(10, 0),
            std_error_1d=std_error
        )

    def predict_forward_returns(self, df: pd.DataFrame, current_percentile: float, current_rsi_ma: float) -> ForwardReturnPrediction:
        """
        Generate comprehensive forward return prediction using all methods.
        """
        ...
        # 2. Markov forecast
        markov_1d = self.markov_forecast(current_bin, 1)
        markov_5d = self.markov_forecast(current_bin, 5)
        markov_10d = self.markov_forecast(current_bin, 10)

        # 3. Regression forecasts
        X_new = np.array([[current_percentile]])

        # Linear
        lr_1d = self.regression_models.get('linear_1d')
        if lr_1d:
            lr_forecast_1d = lr_1d['model'].predict(X_new)[0]
            lr_forecast_5d = self.regression_models.get('linear_5d', {}).get('model', lr_1d['model']).predict(X_new)[0]
            lr_forecast_10d = self.regression_models.get('linear_10d', {}).get('model', lr_1d['model']).predict(X_new)[0]

            linear_reg = RegressionForecast(
                model_type='linear',
                coefficients=lr_1d['model'].coef_.tolist(),
                r_squared=lr_1d['r2'],
                mae=lr_1d['mae'],
                forecast_1d=lr_forecast_1d,
                forecast_5d=lr_forecast_5d,
                forecast_10d=lr_forecast_10d,
                confidence_interval_95=(lr_forecast_1d - 1.96 * lr_1d['mae'], lr_forecast_1d + 1.96 * lr_1d['mae'])
            )
        else:
            linear_reg = None

        # Polynomial
        poly_1d = self.regression_models.get('polynomial_1d')
        if poly_1d:
            X_poly = poly_1d['poly_transformer'].transform(X_new)
            poly_forecast_1d = poly_1d['model'].predict(X_poly)[0]
            poly_forecast_5d = self.regression_models.get('polynomial_5d', {}).get('model', poly_1d['model']).predict(X_poly)[0]
            poly_forecast_10d = self.regression_models.get('polynomial_10d', {}).get('model', poly_1d['model']).predict(X_poly)[0]

            polynomial_reg = RegressionForecast(
                model_type='polynomial',
                coefficients=poly_1d['model'].coef_.tolist(),
                r_squared=poly_1d['r2'],
                mae=poly_1d['mae'],
                forecast_1d=poly_forecast_1d,
                forecast_5d=poly_forecast_5d,
                forecast_10d=poly_forecast_10d,
                confidence_interval_95=(poly_forecast_1d - 1.96 * poly_1d['mae'], poly_forecast_1d + 1.96 * poly_1d['mae'])
            )
        else:
            polynomial_reg = None

        # Quantile regressions
        qr_median = self.regression_models.get('quantile_0.5_1d')
        qr_05 = self.regression_models.get('quantile_0.05_1d')
        qr_95 = self.regression_models.get('quantile_0.95_1d')
        ...
        quantile_median = RegressionForecast(
            ...
            r_squared=0,
            mae=0,
            forecast_1d=qr_median['model'].predict(X_new)[0] if qr_median else 0,
            forecast_5d=self.regression_models.get('quantile_0.5_5d', {}).get('model', qr_median['model']).predict(X_new)[0] if qr_median else 0,
            forecast_10d=self.regression_models.get('quantile_0.5_10d', {}).get('model', qr_median['model']).predict(X_new)[0] if qr_median else 0,
        ...
        quantile_05_reg = RegressionForecast(
            ...
            r_squared=0,
            mae=0,
            forecast_1d=qr_05['model'].predict(X_new)[0] if qr_05 else 0,
            forecast_5d=0,
            forecast_10d=0,
            confidence_interval_95=(0, 0)
        )
        ...
        quantile_95_reg = RegressionForecast(
            ...
            r_squared=0,
            mae=0,
            forecast_1d=qr_95['model'].predict(X_new)[0] if qr_95 else 0,
            forecast_5d=0,
            forecast_10d=0,
            confidence_interval_95=(0, 0)
        )
        ...
        # 4. Kernel forecast
        kernel_pred = self.kernel_forecast(df, current_percentile, 1, bandwidth=10.0)

        # 5. Ensemble (average of all methods)
        forecasts_1d = [
            empirical.mean_return_1d if empirical else 0,
            markov_1d,
            linear_reg.forecast_1d if linear_reg else 0,
            polynomial_reg.forecast_1d if polynomial_reg else 0,
            quantile_median.forecast_1d,
            kernel_pred.forecast_1d
        ]

        forecasts_5d = [
            empirical.mean_return_5d if empirical else 0,
            markov_5d,
            linear_reg.forecast_5d if linear_reg else 0,
            polynomial_reg.forecast_5d if polynomial_reg else 0,
            quantile_median.forecast_5d,
            kernel_pred.forecast_5d
        ]

        forecasts_10d = [
            empirical.mean_return_10d if empirical else 0,
            markov_10d,
            linear_reg.forecast_10d if linear_reg else 0,
            polynomial_reg.forecast_10d if polynomial_reg else 0,
            quantile_median.forecast_10d,
            kernel_pred.forecast_10d
        ]

        ensemble_1d = np.mean([f for f in forecasts_1d if not np.isnan(f)])
        ensemble_5d = np.mean([f for f in forecasts_5d if not np.isnan(f)])
        ensemble_10d = np.mean([f for f in forecasts_10d if not np.isnan(f)])

        return ForwardReturnPrediction(
            current_percentile=current_percentile,
            current_rsi_ma=current_rsi_ma,
            empirical_bin_stats=empirical,
            markov_forecast_1d=markov_1d,
            markov_forecast_5d=markov_5d,
            markov_forecast_10d=markov_10d,
            ...
            ensemble_forecast_1d=ensemble_1d,
            ensemble_forecast_5d=ensemble_5d,
            ensemble_forecast_10d=ensemble_10d
        )
        ...
        for start in range(len(df) - train_window - test_window):
            ...
            for _, row in test_df.iterrows():
                ...
                result = {
                    'date': row['date'],
                    'percentile': pct,
                    'actual_1d': row.get('ret_1d', np.nan),
                    'actual_5d': row.get('ret_5d', np.nan),
                    'actual_10d': row.get('ret_10d', np.nan),
                    'predicted_1d': pred.ensemble_forecast_1d,
                    'predicted_5d': pred.ensemble_forecast_5d,
                    'predicted_10d': pred.ensemble_forecast_10d,
                    'empirical_1d': pred.empirical_bin_stats.mean_return_1d if pred.empirical_bin_stats else np.nan,
                    'markov_1d': pred.markov_forecast_1d,
                    'linear_1d': pred.linear_regression.forecast_1d if pred.linear_regression else np.nan,
                    'kernel_1d': pred.kernel_forecast.forecast_1d
                }
...
def run_percentile_forward_analysis(ticker: str, lookback_days: int = 1095) -> Dict:
    ...
    mapper = PercentileForwardMapper(horizons=[1, 5, 10])
    ...
    for bin_idx, stats in bin_stats.items():
        print(f"    {stats.bin_label}: n={stats.count}, "
              f"E[R_1d]={stats.mean_return_1d:+.2f}%, "
              f"E[R_5d]={stats.mean_return_5d:+.2f}%, "
              f"E[R_10d]={stats.mean_return_10d:+.2f}%")
    ...
    print(f"\n  Ensemble Forecast:")
    print(f"    1-day:  {prediction.ensemble_forecast_1d:+.2f}%")
    print(f"    5-day:  {prediction.ensemble_forecast_5d:+.2f}%")
    print(f"    10-day: {prediction.ensemble_forecast_10d:+.2f}%")

    print(f"\n  Method Breakdown (1-day):")
    if prediction.empirical_bin_stats:
        print(f"    Empirical:  {prediction.empirical_bin_stats.mean_return_1d:+.2f}%")
    print(f"    Markov:     {prediction.markov_forecast_1d:+.2f}%")
    if prediction.linear_regression:
        print(f"    Linear:     {prediction.linear_regression.forecast_1d:+.2f}%")
    if prediction.polynomial_regression:
        print(f"    Polynomial: {prediction.polynomial_regression.forecast_1d:+.2f}%")
    print(f"    Kernel:     {prediction.kernel_forecast.forecast_1d:+.2f}%")
